...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        for(int i = 0; i < v && StdIn.hasNextLine(); i++){
        ...
        for(int i = 0; i<e && StdIn.hasNextLine(); i++){
            String f = StdIn.readString();
            String t = StdIn.readString();
            int from = getIndex(f, neurons);
            int to = getIndex(t, neurons);
            matrix[from][to] = 1;
        }

        //find mind stone vertex
        boolean allZeros = false;
        int mindStone = 0;
        for(int r = 0; r<v && allZeros == false ; r++){
            allZeros = true;
            for(int c = 0; c<v && allZeros == true; c++){
                if(matrix[r][c]!=0){
                    allZeros = false;
                }
            }
        ...
        //find mind stone neighbors
        for(int i = 0; i<v; i++){
            if(matrix[i][mindStone]==1){
                StdOut.println(neurons[i]);
            }
        }
        
    }
    private static int getIndex(String s, String[] a){
